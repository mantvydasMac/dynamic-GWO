...
using GWO.Optimizer.Populations.PSO;
using GWO.Util;

namespace GWO.Experiments.GWO;

public class MPSO_Rosenbrock
{
    public static void Run(string savePath)
    {
        ...
        var function = new Rosenbrock(dimension, ShiftLinear);
        
        var population = new MPSO(function, 1, 1, 25, 5, convergenceRadiusParameter: 0.06);
        ...
        for (int i = 0; i < runCount; ++i)
        {
            ...
            ResultExporter.SaveResultsToCsv(res, savePath + folderPath + $"results_rosenbrock_linear_{i+1}.csv");
            errorStats.Add((i + 1, offlineError, trackingError));
        }
        Console.WriteLine("++ FINISHED LINEAR");
        
        SaveErrorStats(savePath + folderPath + "errors_rosenbrock_linear.csv", errorStats);
        ...
        {
            (offlineError, trackingError, res) = population.Process(iterationNumber);
            ResultExporter.SaveResultsToCsv(res, savePath + folderPath + $"results_rosenbrock_teleport_{i+1}.csv");
            errorStats.Add((i + 1, offlineError, trackingError));
        }
        Console.WriteLine("++ FINISHED TELEPORT");

        SaveErrorStats(savePath + folderPath + "errors_rosenbrock_teleport.csv", errorStats);
        ...
        {
            (offlineError, trackingError, res) = population.Process(iterationNumber);
            ResultExporter.SaveResultsToCsv(res, savePath + folderPath + $"results_rosenbrock_periodic_{i+1}.csv");
            errorStats.Add((i + 1, offlineError, trackingError));
        }
        Console.WriteLine("++ FINISHED PERIODIC");
        
        SaveErrorStats(savePath + folderPath + "errors_rosenbrock_periodic.csv", errorStats);
