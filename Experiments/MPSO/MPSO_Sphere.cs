...
public class MPSO_Sphere
{
    public static void Run(string savePath)
    {
        ...
        var function = new Sphere(dimension, ShiftLinear);
        ...
        for (int i = 0; i < runCount; ++i)
        {
            ...
            ResultExporter.SaveResultsToCsv(res, savePath + folderPath + $"results_sphere_linear_{i+1}.csv");
            errorStats.Add((i + 1, offlineError, trackingError));
        }
        Console.WriteLine("++ FINISHED LINEAR");
        
        SaveErrorStats(savePath + folderPath + "errors_sphere_linear.csv", errorStats);
        ...
        {
            (offlineError, trackingError, res) = population.Process(iterationNumber);
            ResultExporter.SaveResultsToCsv(res, savePath + folderPath + $"results_sphere_teleport_{i+1}.csv");
            errorStats.Add((i + 1, offlineError, trackingError));
        }
        Console.WriteLine("++ FINISHED TELEPORT");

        SaveErrorStats(savePath + folderPath + "errors_sphere_teleport.csv", errorStats);
        ...
        {
            (offlineError, trackingError, res) = population.Process(iterationNumber);
            ResultExporter.SaveResultsToCsv(res, savePath + folderPath + $"results_sphere_periodic_{i+1}.csv");
            errorStats.Add((i + 1, offlineError, trackingError));
        }
        Console.WriteLine("++ FINISHED PERIODIC");
        
        SaveErrorStats(savePath + folderPath + "errors_sphere_periodic.csv", errorStats);
